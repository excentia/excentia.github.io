...
<h2>¿Qué es Gitflow?</h2>
 ...
 Gitflow es un modelo de utilización de ramas para Git y que fue creado en 2010 por [Vincent Driessen](https://github.com/nvie). 
 
<h2>¿Cuáles son las características de Gitflow?</h2>
 
El modelo Gitflow ofrece una serie de ventajas para los equipos de desarrollo que nombraremos a continuación:
...
Git es un <strong>sistema de control de versiones</strong> con soporte de ramas. Hay que entender un repositorio Git como las ramas de un árbol. Existirá un primer commit de código en una rama principal y posteriormente, saldrán de ahí las ramas que sean necesarias. Cada rama es una <strong>derivación desde la rama principal</strong>. Haremos una serie de cambios en dicha rama y podrá volver o no a encontrarse (mergear) contra la rama principal.


## Ramas principales

Gitflow establece que existen **dos ramas principales** dentro de un repositorio: develop y master o main.

<strong>- Develop</strong>: Rama más interesante de cara al desarrollador. Contendrá los últimos cambios finalizados en nuestro software. Es, por lo tanto, la rama principal de desarrollo.


<strong>- Main</strong>: Rama que va a contener el último estado del desarrollo que consideramos que está preparado para producción. Generalmente sólo se mergea contra esta rama cuando generemos una versión para producción del software.
...
Una de las ventajas que ofrece Gitflow es que nos da la opción de testear (y debemos) y hacer todos los cambios necesarios en la rama develop antes de enviar un paquete a producción, mediante la rama main. De esta forma, separamos el código que está 100% listo para producción de las versiones que pueden no estar estables, asegurandonos una mayor tolerancia a fallos.


### Flujo de trabajo - Ramas de soporte

Un desarrollador tiene que realizar una tarea. Para la realización de la misma creará nuevas ramas con reglas estrictas de nomenclatura, rama de origen y rama a la que mergearán.
...
Cuando se finalice la característica correctamente, **esta rama se mergeará contra develop**, preferiblemente aunando todos los posibles commits de la rama feature en uno sólo en la develop. Esto se conoce como squash. 

Recomendaciones para estas ramas:
<br>
- Estas ramas son ramas de corta duración, deben contener los mínimos cambios imprescindibles.
<br>
...
Una vez mergeada la rama, haremos una etiqueta o tag con el número de versión en el último commit realizado en la main. Será esta tag la que enviaremos a producción.
...
No preocuparse, crearemos una rama con nomenclatura **hotfix/(descripcion_del_fallo)** que saldrá desde el último commit de la rama main. Realizaremos la corrección necesaria sobre dicha rama. Haremos todas las pruebas necesarias. Una vez terminado el cambio tenemos que mergear contra dos ramas diferentes:

<strong>Main</strong>: Como la intención es enviar el cambio lo más rápido a producción posible, mergearemos directamente contra la rama main. Es por ello que:<br>
...
Por supuesto, al mergear esta rama haremos squash de los commits, para incorporar un sólo commit. Una vez mergeada la rama contra master se generará una nueva tag que será la versión que desplegaremos. Personalmente creo que lo mejor es llamar a la tag **numero.de.version-hf1**. La rama se eliminará tras haber mergeado contra develop.
